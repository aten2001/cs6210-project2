...
void MPI_tournament_barrier(MPI_Comm comm, int tag) {
  ...
  if (my_id == 0) {
    ...
    for (i = 1; i < num_processes; i <<= 1) {
      MPI_Recv(NULL, 0, MPI_DATATYPE_NULL, i, tag, comm, NULL);
    }
    // All losers have sent message, so now wakeup losers
    for (i >>= 1; i > 0; i >>= 1) {
      MPI_Isend(NULL, 0, MPI_DATATYPE_NULL, i, tag, comm, NULL);
    }
  } else {
    int win_stride = 1;
    // Winner wait loop
    while ((my_id % (2 * win_stride)) == 0) {
      ...
      MPI_Recv(NULL, 0, MPI_DATATYPE_NULL, my_id + win_stride, tag, comm, NULL);
      win_stride *= 2;
    }

    // Send message to winner
    MPI_Send(NULL, 0, MPI_DATATYPE_NULL, my_id - win_stride, tag, comm);
    ...
    while (win_stride > 1) {
      win_stride /= 2;
      MPI_Isend(NULL, 0, MPI_DATATYPE_NULL, my_id + win_stride, tag, comm, NULL);
    }
  }
}